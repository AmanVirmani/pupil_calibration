...
public:
	...
	std::cout << "DVS image size: " << e_undistorted.rows << ", " << e_undistorted.cols << std::endl;
        ...
        cv::resize(c_undistorted, c_resized, cv::Size(c_image.cols, c_image.rows), 0, 0);
        cv::resize(e_undistorted, e_resized, cv::Size(c_image.cols, c_image.rows), 0, 0);
	std::cout << "RGB image size: " << c_resized.rows << ", " << c_resized.cols << std::endl;
	std::cout << "DVS image size: " << e_resized.rows << ", " << e_resized.cols << std::endl;
    ...
    void callback_gaze(const pupil_ros::gaze_positions &info) {
        ...
        this->point_Z = info.gazes[0].gaze_point_3d.z; 
	if (this->point_Z) {
		this->point_Z = 0.5;
	}
