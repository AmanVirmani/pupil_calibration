...
protected:
    ...
    CircularArray<Event, 300000, 10000000> ev_buffer;
...
public:
    ...
    void event_cb(const T& msg) {
        ...
        for (uint i = 0; i < msg->events.size(); i+=10) {
...
int main (int argc, char** argv) {
    ...
    if (!nh.getParam("dir", result_path)) {
        std::cout << _red("Output directory needs to be specified!") << " (_dir:=<...>)\n";
        return -1;
    }
    ...
    // parse config
    std::string path_to_self = ros::package::getPath("cross_calib");
    std::string config_path = "";
    if (!nh.getParam("conf", config_path)) config_path = path_to_self + "/collect.cfg";
    std::cout << _blue("Using conf file at: ") << config_path << "\n";
